...
def preview_message(payload: ChatPreviewRequest, db: Session = Depends(get_db)) -> ChatPreviewResponse:
    try:
        return ChatService(db).preview_message(payload)
    except SessionNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc


@router.post("/confirm", response_model=AssistantReplyResponse, status_code=status.HTTP_201_CREATED)
def confirm_message(payload: ChatConfirmRequest, db: Session = Depends(get_db)) -> AssistantReplyResponse:
    try:
        return ChatService(db).confirm_message(payload)
    except SessionNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
